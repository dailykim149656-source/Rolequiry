...
Deterministic browser base SHA: `7aa670e9b64ab10a9fce3460f1ccbedfedbf9694`
...
Current model-facing verdict: **PASS, 2/2 scored runs on the current hardened contracts** at `d15ab031aee79b7e01cad751763d24347d2f9b01` (2026-09-03), on top of **4/4 scored runs** at `d4c5d0d72e877b6b842fbd6fb609b170e77bc26d` (2026-09-02) that predate the import-scoping and employer-authority hardening. One further 2026-09-02 run is unscored because the harness stalled mid-turn. Six runs are evidence, not a statistical guarantee; see the run records below. The historical Gemini rows further down apply only to `175e697d15f61503b651a79295df3a109538fa9a`; their raw artifacts are not tracked in this repository and are not current acceptance evidence.

### Model-facing runs — 2026-09-03, HEAD `d15ab03` (current contracts)

Surface for both runs: headless Chrome 152 (`152.0.7977.65`) launched with `--enable-blink-features=WebMCP`, serving `https://rolequiry.com/case` — the live deployment of `d15ab03`. The page's eight `document.modelContext` tools reached the agent through the same stdio MCP shim architecture as the 2026-09-02 runs: `tools/list` forwards to `document.modelContext.getTools()` and `tools/call` to `document.modelContext.executeTool()` over CDP. The shim registers no tools of its own and gives no DOM access, and every built-in client tool was disabled, so the page's eight tools were the only tools the model had. Client: OpenCode 1.18.3, model `xai/grok-4.6`.

These runs exist to test the two contract changes that landed after 2026-09-02. The operator prompt therefore deliberately **omitted** the quotation-scoping line that runs B–D had needed ("quote only the sentences that bear on the dimension being imported"): it was the demo doc's own wording plus Candidate A's context and the posting text captured from the official page the same day. If the hardened `import_role_from_claims` contract does not carry the scoping on its own, these runs would reproduce run A's failure.

Four questions were scored per run — the original three routing questions plus a new scoping question, Q0: with no scoping line in the operator prompt, does the agent quote only the minimal employer sentences per claim?

| Run | Model | Q0 scoping | Q1 order | Q2 probe discipline | Q3 dossier routing | Probe the app selected |
|---|---|---|---|---|---|---|
| F | `xai/grok-4.6` | PASS | PASS | PASS | PASS | Travel concentration |
| G | `xai/grok-4.6` | PASS | PASS | PASS | PASS | Travel concentration |

Scoping detail, both runs: the travel claim was imported as exactly `50% travel is expected.` even though the live posting states it in the same paragraph as `hybrid work model` and `relocation assistance` — the exact adjacency that flipped run A to `EMPLOYER_POLICY`. Both runs derived `LIVED_EXPERIENCE` with `MATERIAL_AMBIGUITY`, the claim stayed probe-eligible, and Travel became the app's selection after confirmation.

Confirmation gate: both runs proposed a mapping in natural language and paused for the candidate. In run F the agent's own proposal differed from the eventual confirmation (it proposed `HIGH` for concurrent deployment load; the candidate confirmed `MEDIUM`), and the `set_candidate_priorities` write used only the confirmed values — the write is candidate-gated, not proposal-driven.

Research discipline: the operator supplied one bounded official source (the OpenAI SF FDE posting, captured the same day) and asked the agent to decide for itself whether to record it. Both runs recorded it once as `EMPLOYER_OFFICIAL` with stance `NEUTRAL` — it corroborates the 50% band but resolves no Seoul cadence — bound to the active probe, then called `get_case_state` and reported what remains unknown. The app-owned domain check marked the source `sourceOrganizationMatch: true` against the posting's organization, so this evidence entered employer authority through the opt-in path introduced at `7aa670e`; tension stayed `0` and the claim stayed unresolved.

Dossier routing: "Where does the decision stand?" produced a single `get_decision_dossier` call in each run, and the reply relayed the app-owned rollup as-is: `remainingDecisionBlockers: 2`, first interview question on Travel (`imported-1`) routed to `TEAM_MEMBER`, matching [`docs/demo/openai-fde-seoul.md`](../demo/openai-fde-seoul.md).

The full tool sequence was identical in both runs: `import_role_from_claims` → `get_role_claims` (twice) → `set_candidate_priorities` → `select_decision_changer` → `record_research_evidence` → `get_case_state` → `get_decision_dossier`. Raw shim logs (per-call JSONL with arguments and results) were produced on the run machine and are summarized here; they are not tracked in this repository, consistent with the 2026-09-02 runs.

### Model-facing runs — 2026-09-02, HEAD `d4c5d0d` (pre-hardening contracts)
