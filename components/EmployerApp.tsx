...
export function EmployerApp() {
  ...
  useWebMCP({
    ...
    description: "Return Atlas Robotics official claims for the Forward Deployed Engineer role. Employer-authored testimony, not verified facts.",
  ...
  useWebMCP({
    ...
    description: "Return official employer policy statements for compensation and related policy claims.",
    inputSchema: emptySchema,
    annotations: { readOnlyHint: true, untrustedContentHint: true },
    execute: () => ({
      company: ATLAS_FDE.company,
      ...
      policyClaims: ATLAS_FDE.claims
        .filter((claim) => claim.kind === "EMPLOYER_POLICY" || claim.id === "compensation")
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <p className="text-sm tracking-[0.2em] text-zinc-500">EMPLOYER SURFACE</p>
      <h1 className="mt-2 text-3xl font-semibold">{ATLAS_FDE.company}</h1>
      <p className="mt-2 text-zinc-600">{ATLAS_FDE.role}. This is what the employer officially publishes to the agent.</p>
