...
describe("Fixture A deriveCase", () => {
  it("selects technical ownership initially even though travel has stronger tension", () => {
    ...
    const ownership = derived.claims.find((claim) => claim.id === "technical-ownership")
    const travel = derived.claims.find((claim) => claim.id === "travel")
    const compensation = derived.claims.find((claim) => claim.id === "compensation")
    const customer = derived.claims.find((claim) => claim.id === "customer-interaction")
    ...
    expect((compensation?.probePriority ?? 1) < (ownership?.probePriority ?? 0)).toBe(true)

    expect(derived.topProbeId).toBe("technical-ownership")
    expect((ownership?.probePriority ?? 0) - (travel?.probePriority ?? 0)).toBeGreaterThanOrEqual(0.15)
  })
  ...
  it("flips the selected probe when travel is raised to CRITICAL", () => {
    ...
    expect((travel?.probePriority ?? 0) - (ownership?.probePriority ?? 0)).toBeGreaterThanOrEqual(0.05)
  })
