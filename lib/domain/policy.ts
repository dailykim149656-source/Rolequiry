...
export function deriveClaimKind(input: {
  ...
  const haystack = `${input.dimension} ${input.employerStatement}`.toLowerCase()
...
export function requiredScopes(kind: ClaimKind): readonly AuthorityScope[] {
  if (kind === CLAIM_KIND.EMPLOYER_POLICY) return [AUTHORITY_SCOPE.EMPLOYER_STATED]
  return [
...
export function unresolvednessFor(kind: ClaimKind, evidence: readonly Evidence[]): number {
  if (kind === CLAIM_KIND.EMPLOYER_POLICY) {
    const covered = evidence.some((item) => item.scope === AUTHORITY_SCOPE.EMPLOYER_STATED)
    return covered ? 0 : 1
  }

  const reports = evidence.filter((item) => item.scope === AUTHORITY_SCOPE.REPORTED_EXPERIENCE)
  const hasEmployer = evidence.some((item) => item.scope === AUTHORITY_SCOPE.EMPLOYER_STATED)
  const hasAnswer = evidence.some((item) => item.scope === AUTHORITY_SCOPE.CANDIDATE_SPECIFIC_ANSWER)
  const covered =
    (hasEmployer ? LIVED_EXPERIENCE_WEIGHT.EMPLOYER_STATED : 0) +
    LIVED_EXPERIENCE_WEIGHT.REPORTED_EXPERIENCE * reportedCoverage(reports.length) +
...
export function tensionFor(evidence: readonly Evidence[]): number {
  const hasChallengingAnswer = evidence.some(
    (item) => item.scope === AUTHORITY_SCOPE.CANDIDATE_SPECIFIC_ANSWER && item.stance === "CHALLENGES",
  )
  if (hasChallengingAnswer) return 1
  const challengingReports = evidence.filter(
    (item) => item.scope === AUTHORITY_SCOPE.REPORTED_EXPERIENCE && item.stance === "CHALLENGES",
  ).length
...
export function claimStatus(input: {
  ...
  if (IMPORTANCE_WEIGHT[input.importance] >= 0.75 && input.unresolvedness >= 0.35) {
...
export function probePriority(input: {
  ...
  return {
    probeEligible,
    probePriority:
      0.4 * IMPORTANCE_WEIGHT[input.importance] + 0.3 * input.unresolvedness + 0.3 * input.tension,
  }
}
