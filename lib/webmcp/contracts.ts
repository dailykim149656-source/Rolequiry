export const CASE_TOOL_CONTRACTS = [
  ...
  },
  {
    ...
    description:
      "Decision-directed research only: when the candidate naturally approves research, investigate only the currently active probe, not the whole company. Record one agent-reported public employer-published or first-person source with provenance only when it is credible and claim-specific. Stance is relative to the active employer claim, never the candidate's preference or constraint. Before choosing a strong SUPPORTS or CHALLENGES stance, seek credible counterevidence; use NEUTRAL when the source is genuinely non-resolving or mixed. An employer-published source counts toward employer authority only when its domain matches the job posting's organization; mismatched or unverifiable employer sources are recorded but excluded from that authority in the derived decision state. After a successful write, call get_case_state and explain what remains unknown. Never require a tool name or claim ID from the candidate. Do not choose derived decision fields.",
    annotations: { readOnlyHint: false },
  },
  {
